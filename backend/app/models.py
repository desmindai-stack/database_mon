...
class Instance(Base):
    ...
    # Faz 31 Commit 6: ÖLÇÜLEN topoloji (services/server_topology.py). Yapılandırma alanları
    # (cluster_name, services) topoloji DEĞİL — tek sunucuda cluster alarmının kök nedeni buydu.
    topology_kind: Mapped[str | None] = mapped_column(String(16), nullable=True)
    topology_state: Mapped[str | None] = mapped_column(String(16), nullable=True)
    topology_role: Mapped[str | None] = mapped_column(String(16), nullable=True)
    topology_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    topology_required_grant: Mapped[str | None] = mapped_column(Text, nullable=True)
