...
async def migrate_schema() -> None:
    ...
    async with engine.begin() as conn:
        ...
        await _sqlite_add_column_if_missing(conn, "users", "password_changed_at", "password_changed_at DATETIME")
        ...
        # Faz 29 İŞ 2a: pg_stat_statements'ın kalan sütunları (SQLite tarafı).
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "stddev_time_ms", "stddev_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "min_time_ms", "min_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "max_time_ms", "max_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "shared_blks_dirtied", "shared_blks_dirtied BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "shared_blks_written", "shared_blks_written BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "blk_read_time_ms", "blk_read_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "blk_write_time_ms", "blk_write_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "temp_blk_read_time_ms", "temp_blk_read_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "temp_blk_write_time_ms", "temp_blk_write_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "wal_records", "wal_records BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "wal_fpi", "wal_fpi BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "wal_bytes", "wal_bytes FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "plans", "plans BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "total_plan_time_ms", "total_plan_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "jit_time_ms", "jit_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "jit_functions", "jit_functions BIGINT")
        # Faz 29 İŞ 2c: motordan bağımsız sorgu metrikleri (SQL Server DMV'leri).
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "cpu_time_ms", "cpu_time_ms FLOAT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "logical_reads", "logical_reads BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "physical_reads", "physical_reads BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "logical_writes", "logical_writes BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "spills", "spills BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "grant_kb", "grant_kb BIGINT")
        await _sqlite_add_column_if_missing(conn, "slow_query_samples", "used_grant_kb", "used_grant_kb BIGINT")
        await _sqlite_add_column_if_missing(conn, "backup_probes", "recovery_models", "recovery_models JSON")
        await _sqlite_add_column_if_missing(conn, "health_reports", "suppression", "suppression JSON")
